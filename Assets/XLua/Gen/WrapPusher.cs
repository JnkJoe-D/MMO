...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderXLuaTestPedding
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<ViewId>(translator.PushViewId, translator.Get, translator.UpdateViewId);
        ...
        int ViewId_TypeID = -1;
		int ViewId_EnumRef = -1;
        
        public void PushViewId(RealStatePtr L, ViewId val)
        {
            if (ViewId_TypeID == -1)
            {
			    bool is_first;
                ViewId_TypeID = getTypeId(L, typeof(ViewId), out is_first);
				
				if (ViewId_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(ViewId));
				    ViewId_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, ViewId_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, ViewId_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for ViewId ,value="+val);
            }
			
			LuaAPI.lua_getref(L, ViewId_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out ViewId val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != ViewId_TypeID)
				{
				    throw new Exception("invalid userdata for ViewId");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for ViewId");
                }
				val = (ViewId)e;
            ...
            {
                val = (ViewId)objectCasters.GetCaster(typeof(ViewId))(L, index, null);
            }
        }
		
        public void UpdateViewId(RealStatePtr L, int index, ViewId val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for ViewId ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(ViewId[]))
			{
			    ViewId[] array = obj as ViewId[];
				translator.PushViewId(L, array[index]);
				return true;
			}
            return false;
		}
