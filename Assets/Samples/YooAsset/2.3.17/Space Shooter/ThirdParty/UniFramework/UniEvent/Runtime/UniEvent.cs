...
namespace UniFramework.Event
{
	public static class UniEvent
	{
		...
		private static bool _isInitialize = false; //标记事件系统是否初始化
		private static GameObject _driver = null; //驱动MonoBehaviour的GameObject，用于每帧更新
		private static readonly Dictionary<int, LinkedList<Action<IEventMessage>>> _listeners
		= new Dictionary<int, LinkedList<Action<IEventMessage>>>(1000);//存储事件ID和对应的监听者链表（Action<IEventMessage>）
		private static readonly List<PostWrapper> _postingList = new List<PostWrapper>(1000); //存储延迟发送的事件包装列表
		...
		internal static void Update()
		{
			for (int i = _postingList.Count - 1; i >= 0; i--)
			{
				...
				//如果当前帧数大于包装中记录的帧数（即下一帧），则执行发送并移除
				{
		...
		{
			//使用链表存储监听者，确保同一个事件ID可以添加多个监听者，且不会重复
