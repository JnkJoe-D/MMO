...
public class AssetBundleLoader
{
    ...
    public void LoadMainfest()
    {
        ...
        _dependsDataList.Clear();                          //清空子包依赖关系配置缓存
        AssetBundle ab = AssetBundle.LoadFromFile(path);  //加载主包
        ...
        AssetBundleManifest mainfest = ab.LoadAsset("AssetBundleManifest") as AssetBundleManifest;  //获取子包依赖关系配置信息
        if (mainfest == null)
        {
        ...
        ab.Unload(true); //获取完依赖配置销毁主包，节省内存
        ab = null;
    ...
    {//读者可以自己实现的对应关系，笔者这里有多语言和文件版本的处理
        ...
        return PathUrl+GetFileName(_hashName);
    }
    ...
    private AssetBundleObject LoadAssetBundleSync(string _hashName)
    {
        ...
        else if (_loadingABList.ContainsKey(_hashName)) //在异步的加载中, 将异步则改同步
        {
        ...
        else if (_readyABList.ContainsKey(_hashName)) //在异步的准备加载中，直接执行加载，同步没有ready和loading
        {
        ...
        {
            abObj._dependLoadingCount = 0;  //同步依赖的loading计数,无需检查依赖loading清空
                                            //加载完自身后立即进入递归加载依赖，依赖加载完才会return自身，因此return时依赖以及自身都是加载完成的
    ...
    {
        //提取ab
        if (abObj._request != null) //不为null即是没加载完
        {
            abObj._ab = abObj._request.assetBundle; //如果没加载完，此时尝试获取ab包，会阻塞主线程等待加载完后返回ab包，相当于会异步转同步
            abObj._request = null; // 转同步加载完后取消异步请求，避免重复加载
